...
create_csv <- function(start_date, end_date, ferien_data, output_filename) {
  ...
  df <- data.frame(Tag = date_sequence, Ferien = 0)
  ...
  for (ferien_range in ferien_data) {
    ...
    df$Ferien[df$Tag >= start_date & df$Tag < end_date] <- 1
  }
...
# Lese die CSV-Datei
ferientage_data <- read.csv(output_filename)

# Lese die CSV-Datei
ferientage_data <- read.csv(output_filename)

# Zeige die Tage ohne Ferien
tage_ohne_ferien <- subset(ferientage_data, Ferien == 0)
print(tage_ohne_ferien)

# Zeige die Tage mit Ferien
tage_mit_ferien <- subset(ferientage_data, Ferien == 1)
print(tage_mit_ferien)
```

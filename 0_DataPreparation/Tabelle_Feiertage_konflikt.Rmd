...
model_1 <- lm(Umsatz ~ as.factor(Wochentag) + Temperatur + Bewoelkung, data = train_data %>% filter(Warengruppe == "1"))
summary(model_1)
...
```{r}
valid_data$predUmsatz <- predict(model, valid_data)
...
# exportiere als .csv datei in github repository
write.csv(valid_data_pred, "valid_data_pred.csv", row.names = FALSE)

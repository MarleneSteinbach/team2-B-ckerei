...
```{r}
ourdata_fussball <- left_join(feiertage_ferientage, Fussballspiele, by = "Datum")
...
model_1v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur + as.factor(Jahreszeit) + as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien + Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_2v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur * as.factor(Jahreszeit) * as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien + Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_3v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur * as.factor(Jahreszeit) * as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien * Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_4v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur * as.factor(Jahreszeit) * as.factor(Temperaturkategorie)+ as.factor(Warengruppe) + Fussballspiel + Ferien * Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)
...
```{r}
#levels(train_data_noNA$Jahreszeit)
#levels(valid_data_noNA$Jahreszeit)
##Problembewältigung
# Get factor levels from the training data
#factor_levels <- levels(train_data_noNA$Jahreszeit)

## Ensure the factor levels in validation data match those from training data
#valid_data_noNA$Jahreszeit <- factor(valid_data_noNA$Jahreszeit, levels = factor_levels)

## Now, try making predictions again
#valid_data_noNA$predUmsatz <- predict(model_3, newdata = valid_data_noNA)

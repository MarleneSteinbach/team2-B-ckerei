...
# Testdatensatz (01.08.2018 bis 31.07.2019)
test_data <- ourdata_fussball %>% 
  filter(Datum >= as.Date("2018-08-01") & Datum <= as.Date("2019-07-31"))
...
model_1v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur +  as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien + Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_2v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur *  as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien + Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_3v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur *  as.factor(Temperaturkategorie) + as.factor(Warengruppe) + Fussballspiel + Ferien * Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)

model_4v <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur *  as.factor(Temperaturkategorie)+ as.factor(Warengruppe) + Fussballspiel + Ferien * Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt + Kreuzfahrtschiffe, data = valid_data_noNA)
...
valid_data_noNA$id <- paste0(valid_data_noNA$Datum, valid_data_noNA$Warengruppe)
# entferne die Striche bei Datum
valid_data_noNA$id <- gsub("-", "", valid_data_noNA$id)
...
if (!identical(levels_train, levels_valid)) {
  ...
  valid_data_noNA$Warengruppe <- factor(valid_data_noNA$Warengruppe, levels = levels_train)
}
valid_data_noNA$Warengruppe <- factor(valid_data_noNA$Warengruppe, levels = levels_train)
head(valid_data_noNA)

...
```{r}
model_1 <- lm(Umsatz ~ Wettercode + Bewoelkung + Windgeschwindigkeit + Temperatur +  as.factor(Jahreszeit) + as.factor(Temperaturkategorie) + as.factor(Warengruppe)+ as.factor(Art_des_Feiertags) +Fussballspiel + Ferien + Feiertag  + KielerWoche + as.factor(Wochentag) + Flohmarkt +  Kreuzfahrtschiffe + 
Wettercode * Bewoelkung * Windgeschwindigkeit * Temperatur + 
as.factor(Warengruppe) * as.factor(Art_des_Feiertags) + 
as.factor(Jahreszeit) * as.factor(Temperaturkategorie) + 
...
valid_predictions <- predict(model_1, newdata = valid_data)
...
# Make predictions
test_predictions <- predict(model_1, test_features_for_prediction)

...
# Define a function for conditional interpolation
conditional_interpolation <- function(df) {
  na_indices <- apply(is.na(df), 1, function(row) sum(row) <= 2)
  df[na_indices, ] <- t(apply(df[na_indices, ], 1, zoo::na.approx))
  return(df)
}

# Apply conditional interpolation to selected columns
columns_to_interpolate <- c("Windgeschwindigkeit", "Temperatur")

data_test[columns_to_interpolate] <- conditional_interpolation(data_test[columns_to_interpolate])

# Check the result
print(data_test[c("Datum", "Windgeschwindigkeit", "Temperatur")])
...
donor_vars_wettercode <- c("Temperatur", "Windgeschwindigkeit", "Bewoelkung")
Daten_imputed2 <- hotdeck(data_test, "Bewoelkung")
Daten_imputed2<- hotdeck(data_test, "Wettercode")
...
Daten_imputed3$Datum <- as.Date(Daten_imputed3$Datum)
first_table$Datum <- as.Date(first_table$Datum)
kiwo$Datum <- as.Date(kiwo$Datum)
                                                                
Grundtabelle <- full_join(Daten_imputed3, first_table, by = "Datum") %>%
...
```{r}
donor_vars_bewoelkung <- c("Temperatur", "Windgeschwindigkeit", "Wettercode", "Diff_temp")
donor_vars_wettercode <- c("Temperatur", "Windgeschwindigkeit", "Diff_temp", "Bewoelkung")
Daten_imputed2 <- hotdeck(data_test, "Bewoelkung")
Daten_imputed2<- hotdeck(data_test, "Wettercode")

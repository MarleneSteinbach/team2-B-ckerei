...
```{r}
data <- data %>%
  mutate(Temperatur_imputed = ifelse(row_number() %in% miss_temp_3$Datum, 
                                      as.numeric(zoo::na.approx(Temperatur)), Temperatur),
         Windgeschwindigkeit_imputed = ifelse(row_number() %in% miss_temp_3$Datum, 
                                                as.numeric(zoo::na.approx(Windgeschwindigkeit)), Windgeschwindigkeit))
```

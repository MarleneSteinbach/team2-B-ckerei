...
```
`Testen des Imputationsverfahrens (erstmal nur lineare Regression)
...
# Erstellen Sie eine Kopie Ihrer Daten
Daten_with_NA <- Daten

# Entfernen Sie Werte für die ausgewählten Tage in den spezifischen Spalten
...
# Interpolieren Sie nur die numerischen Spalten
for (col in columns_to_remove) {
...
```{r}
# Benutzerdefinierte Hot-Deck-Imputation für "Wettercode"
hot_deck_imputation <- function(data, variable, donor_vars) {
  # Erstellen Sie eine Kopie der Daten für die Imputation
  imputed_data <- data
  
  # Finden Sie die Zeilen mit fehlenden Werten im "Wettercode"
  missing_indices <- which(is.na(data[[variable]]))
  
  # Für jede fehlende Beobachtung
  for (i in missing_indices) {
    # Wählen Sie die Beobachtungen ohne fehlende Werte im "Wettercode"
    available_data <- data[!is.na(data[[variable]]), ]
    
    # Berechnen Sie die Distanz zwischen der fehlenden Beobachtung und allen verfügbaren Beobachtungen
    distances <- apply(available_data[, donor_vars], 1, function(x) {
      sum((x - data[i, donor_vars])^2, na.rm = TRUE)
    })
    
    # Finden Sie den Index des nächsten Nachbarn (Spender)
    donor_index <- which.min(distances)
    
    # Imputieren Sie den "Wettercode" von der ähnlichsten Beobachtung
    imputed_data[i, variable] <- available_data[donor_index, variable]
  }
  
  return(imputed_data)
}

# Führen Sie die Hot-Deck-Imputation aus
donor_vars <- c("Bewoelkung", "Windgeschwindigkeit", "Temperatur", "Diff_temp", "Diff_wind")
Daten_imputed2 <- hot_deck_imputation(Daten_imputed, "Wettercode", donor_vars)
...
```{r}
Daten_hotdeck <- Daten_imputed %>% hotdeck(variable = "Wettercode", donor_vars = c("Bewoelkung", "Windgeschwindigkeit", "Temperatur", "Diff_temp", "Diff_wind"))
```
...
for (col in columns_to_remove) {
  if (is.numeric(Daten[[col]])) {
    # Erstellen Sie einen Vektor, der TRUE ist, wenn der Wert im Originaldatensatz für die Berechnung verwendet werden soll
    days_to_evaluate <- Daten$Datum %in% days_to_remove
    
    # Wählen Sie nur die Werte aus, die für die Berechnung des Fehlers verwendet werden sollen
    original_values <- Daten[days_to_evaluate, col]
    imputed_values <- Daten_with_NA[days_to_evaluate, col]
    
    # Entfernen Sie NA-Werte, bevor Sie MSE und MAE berechnen
